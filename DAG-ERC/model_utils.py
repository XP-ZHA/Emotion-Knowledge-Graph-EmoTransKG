...
class GatLinear(nn.Module):
    ...
    def forward(self, Q, K, V, adj):
        ...
        # print(attn_weight)
        #torch.bmm：矩阵相乘：一个是(b,h,w),一个是(b,w,m)；输出维度 （b,h,m）
...
class GAT_dialoggcn_v1(nn.Module):
    ...
    def forward(self, Q, K, V, adj, s_mask):
        ...
        '''
        imformation gatherer with linear attention
        :param Q: (B, D) # 公式(5)的query utterance，代表第l-1层节点i/v的特征
        :param K: (B, N, D) # 公式(5)的key，代表第l层节点i/v邻居的特征
        :param V: (B, N, D) # 公式(5)的value，和K一样，代表第l层节点i/v邻居的特征
        :param adj: (B,  N) # adj[:,i,:i]：与节点i连接的节点之前的边，dataset.py的函数  def get_adj(self, speakers, max_dialog_len)
        :param s_mask: (B,  N) #s_mask[:,i,:i]：与节点i连接的节点的说话者信息，其中1表示相同的说话者，0表示不同的说话者
        :return:
        '''
        B = K.size()[0]
        N = K.size()[1]
        ...
        #V0 * s_mask：获取与节点i相同说话人的节点信息
        #V1 * (1 - s_mask)：获取与节点i不相同说话人的节点信息
        V = V0 * s_mask + V1 * (1 - s_mask)
        #torch.bmm矩阵相乘：聚合节点i邻居的信息
        attn_sum = torch.bmm(attn_weight, V).squeeze(1) # (B, D)
        # print('attn_sum',attn_sum.size())
...
class attentive_node_features(nn.Module):
    ...
    def forward(self,features, lengths, nodal_att_type):
        ...
        temp = torch.bmm(x, features.permute(0,2,1)) # (B, N, N)
        #print(temp)
